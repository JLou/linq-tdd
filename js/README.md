...
Vu que vous n'allez travailler que sur une méthode a la fois, je vous conseille de faire `t` puis de taper le nom de la fonction que vous êtes en train d'écrire.
Exemple :
...
Map est une projection. On a en entrée un tableau de `TOuput`: `array`, et une fonction `callback` qui a en entrée un element de type `TOuput` et en sortie un element de type `TOuput`.
On applique cette fonction a l'ensemble des élements du tableau et on obtient un nouveau tableau de `TOuput`.
